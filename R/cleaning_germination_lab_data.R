...
#This code uses the dataDownloader package https://github.com/Between-the-Fjords/dataDownloader. Data downloader downloads data from OSF, and if you already have the file it checks if it is up to date with the version on OSF. This dataset will not change anymore, so you can also directly download the data from OSF (https://osf.io/h7qau/) to your computer and import the data locally from your computer. But I recommend trying datadownloader for working with datasets that are actively being updated.

#osf_auth(token = "personal access token for OSF") 
# 
# get_file(node = "h7qau",
#          file = "INCLINE_Germination_Seedling_Experiment_Data_Va.csv",
#          path = "data",
#          remote_path = "Raw_Data/Germination_lab_experiment")
# 
# get_file(node = "h7qau,
...
harvest_comment_dict_VA <- read_delim("data/harvest_comment_dictionary_VA.csv", delim = ";")
dish_comment_dict_VA <- read_delim("data/dish_comment_dictionary_VA.csv", delim = ";")
comment_dict_SP <- read_delim("data/comment_dictionary_SP.csv", delim = ";")
harvest_comment_dict_SP <- read_delim("data/harvest_comment_dictionary_SP.csv", delim = ";")
weighing_comment_dict_SP <- read_delim("data/weighing_comment_dictionary_SP.csv", delim = ";")
seed_mass_df <- read.delim("data/Seed_weight.csv", sep = ",", dec = ".")
